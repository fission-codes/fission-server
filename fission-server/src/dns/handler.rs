...
impl Handler {
    ...
    async fn do_handle_request_fission<R: ResponseHandler>(
        ...
        match str::from_utf8(hostname)? {
            ...
            "_dnslink" => match request.query().query_type() {
                RecordType::TXT => {
                    let Some(host) = name_iter.next() else { return self.empty_response(request, responder).await };
    ...
    async fn do_handle_request_dnslink<R: ResponseHandler>(
        ...
        let conn = Arc::new(Mutex::new(db::connect(&self.db_pool).await?));

        // FIXME this needs to fetch from apps, not users.
        let account = Account::find_by_username(conn, None, hostname.to_string()).await?;
        let did = account.did;
