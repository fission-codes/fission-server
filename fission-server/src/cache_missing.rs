//! An implementation of a blockstore that remembers which blocks are missing
use anyhow::Result;
use bytes::Bytes;
use cid::Cid;
...
/// A blockstore wrapper that remembers which blocks are missing
#[derive(Debug, Clone)]
pub struct CacheMissing<B: BlockStore> {
    /// Which CIDs are missing
    pub missing_cids_cache: Arc<Cache<Cid, ()>>,
    /// The inner blockstore
    pub inner: B,
}

impl<B: BlockStore> CacheMissing<B> {
    /// Constructs a new caching blockstore that remembers the set of missing CIDs
